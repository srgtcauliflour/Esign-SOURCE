...
// downloaded IPAs, uploaded signing certificates, and signed output never
// touch another visitor's. ipatool stores its state under $HOME/.ipatool,
// and its keychain passphrase has no env-var hook — it must be passed as
// --keychain-passphrase on every invocation (see lib/ipatool.js), so
// IPATOOL_KEYCHAIN_PASSPHRASE here is just this module's own carrier for
...
function ensure(sessionId) {
  ...
  const signed = path.join(home, 'signed');
  ...
  fs.mkdirSync(signed, { recursive: true, mode: 0o700 });
